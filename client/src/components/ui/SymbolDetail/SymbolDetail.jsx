...
function formatSyncedLabel(synced_at, price_source) {
  ...
  if (price_source === "historical") {
    const date = new Date(synced_at).toLocaleDateString("en-US", {
      month: "short",
      day: "numeric",
      year: "numeric",
    });
  ...
  if (error) {
    return (
      <div className="symbol-detail symbol-detail--error">
        <p>
          Failed to load quote for <strong>{symbol}</strong>.
        </p>
  ...
  return (
    <div className="symbol-detail">
      ...
      <div className="symbol-detail__stats">
        ...
        <StatCell
          label="Last Sale"
          value={hasPrice ? `$${price.toFixed(2)}` : "—"}
          skeleton={priceLoading}
        />
        ...
        <StatCell
          label="Shares Out."
          value={formatShares(quote.shares_outstanding)}
        />
